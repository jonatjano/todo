...
let commonPathToIgnore = [
    ...
    '\\*\\*/._\\*/\\*\\*',         '\\*\\*/npm-debug.log/\\*\\*', '\\*\\*/.npmrc/\\*\\*',   '\\*\\*/node_modules/\\*\\*',
    '\\*\\*/config.gypi/\\*\\*',   '\\*\\*/*.orig/\\*\\*',        '\\*\\*/package-lock.json/\\*\\*'
];

let scopes = ['opened files', 'current project', 'current project + opened files', 'projects of opened files', 'workspace'];
...
const Todo = {
  ...
  config: {
    ...
    e_todoScope: {
      title: 'The scope',
      description: 'The scope where we search for todos (workspace stand for every )',
      type: 'string',
      default: 'projects of opened files',
      enum: scopes
    },
  },
